...
export function HomeToolSelector({
  value,
  onChange,
  variant = "default",
}: HomeToolSelectorProps) {
  const { t } = useLanguage();
  ...
  const listbox = open ? (
    <div
      ...
      className="absolute top-full z-40 mt-1 max-h-60 min-w-full w-max max-w-[min(100vw-2rem,22rem)] overflow-y-auto rounded-md border border-border bg-card py-1 shadow-[var(--shadow-elevated)]"
    >
  ...
  const trigger = (
    <button
      ...
      className={`flex w-full items-center justify-between gap-2 rounded-md bg-foreground/[0.06] font-label text-foreground transition-colors hover:bg-foreground/[0.1] ${
        isToolbar
          ? "h-9 px-3 text-sm font-semibold normal-case tracking-normal"
          : "rounded-none border border-border bg-card px-3 py-2.5 text-sm"
      }`}
    >
  ...
  return (
    <div
      ...
      className={
        isToolbar
          ? "embedded-tool-selector w-full min-w-0"
          : "relative w-full max-w-xs space-y-2"
      }
    >
      <label
        id={`${triggerId}-label`}
      ...
      {isToolbar ? (
        <div className="embedded-tool-selector-trigger-wrap">
          {trigger}
          {listbox}
        </div>
      ) : (
