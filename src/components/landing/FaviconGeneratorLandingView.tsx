...
import { FaviconGeneratorLandingRelatedUseCases } from "@/components/landing/FaviconGeneratorLandingRelatedUseCases";
import {
...
import {
  ...
  getFaviconGeneratorLandingEntry,
} from "@/lib/favicongeneratorLandingsLocale";
import { Download, Globe, Image, type LucideIcon } from "lucide-react";
import type { CSSProperties } from "react";

const DEFAULT_BENEFIT_ICONS = [Globe, Image, Download] as const;

interface FaviconGeneratorLandingViewProps {
  landingId: FaviconGeneratorLandingId;
}

export function FaviconGeneratorLandingView({
  landingId,
}: FaviconGeneratorLandingViewProps) {
  ...
  const accent = FAVICON_GENERATOR_LANDING_ACCENT;
  ...
  return (
    ...
    >
      <section className="mb-14 sm:mb-16">
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)] lg:items-end">
          <div className="max-w-2xl space-y-5">
            ...
            >
              <LandingCtaButton href={FAVICON_GENERATOR_TOOL_HREF} accent={accent}>
                {landing.primaryCta}
              </LandingCtaButton>
      ...
      <section className="mb-14 sm:mb-16">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {landing.benefits.map(({ title, body }, index) => {
            const Icon: LucideIcon = DEFAULT_BENEFIT_ICONS[index] ?? Globe;
            return (
              <article
                key={title}
      ...
      <FaviconGeneratorLandingRelatedUseCases currentLandingId={landingId} />
