...
interface ResizerLandingRelatedUseCasesProps {
  currentLandingId: ResizerLandingId;
}

export function ResizerLandingRelatedUseCases({
  currentLandingId,
}: ResizerLandingRelatedUseCasesProps) {
  const { language } = useLanguage();
  const relatedPages = getRelatedResizerLandingPages(
    currentLandingId,
    language,
  );
              ...
              <li key={page.path}>
                <AppLink href={page.path} className={linkCardClassName}>
                  <h3
                    ...
                    style={{ color: RESIZER_LANDING_ACCENT }}
                  >
                    {page.linkTitle}
                  </h3>
