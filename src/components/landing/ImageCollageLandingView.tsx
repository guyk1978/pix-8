...
import { ImageCollageLandingRelatedUseCases } from "@/components/landing/ImageCollageLandingRelatedUseCases";
import {
...
import {
  IMAGE_COLLAGE_LANDING_ACCENT,
  IMAGE_COLLAGE_TOOL_HREF,
  type ImageCollageLandingId,
} from "@/lib/imageCollageLandings";
import {
  getImageCollageLandingChrome,
  getImageCollageLandingEntry,
} from "@/lib/imageCollageLandingsLocale";
import { Image, LayoutGrid, Shield, type LucideIcon } from "lucide-react";
import type { CSSProperties } from "react";

const DEFAULT_BENEFIT_ICONS = [Shield, LayoutGrid, Image] as const;

interface ImageCollageLandingViewProps {
  landingId: ImageCollageLandingId;
}

export function ImageCollageLandingView({
  landingId,
}: ImageCollageLandingViewProps) {
  const { language, dir } = useLanguage();
  const landing = getImageCollageLandingEntry(language, landingId);
  const chrome = getImageCollageLandingChrome(language);
  const accent = IMAGE_COLLAGE_LANDING_ACCENT;
  ...
  return (
    ...
    >
      <section className="mb-14 sm:mb-16">
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)] lg:items-end">
          <div className="max-w-2xl space-y-5">
            ...
            >
              <LandingCtaButton href={IMAGE_COLLAGE_TOOL_HREF} accent={accent}>
                {landing.primaryCta}
              </LandingCtaButton>
      ...
      <ImageCollageLandingRelatedUseCases currentLandingId={landingId} />
