...
import { TextOverlayLandingRelatedUseCases } from "@/components/landing/TextOverlayLandingRelatedUseCases";
import {
...
import {
  TEXT_OVERLAY_LANDING_ACCENT,
  TEXT_OVERLAY_TOOL_HREF,
  type TextOverlayLandingId,
} from "@/lib/textOverlayLandings";
import {
  getTextOverlayLandingChrome,
  getTextOverlayLandingEntry,
} from "@/lib/textOverlayLandingsLocale";
import { ImagePlus, Layers, PenLine, Shield, Sparkles, Type, type LucideIcon } from "lucide-react";
import type { CSSProperties } from "react";

const DEFAULT_BENEFIT_ICONS = [Shield, Type, Sparkles] as const;

const LANDING_BENEFIT_ICONS: Partial<
  Record<TextOverlayLandingId, readonly [LucideIcon, LucideIcon, LucideIcon]>
> = {
  "add-text-on-image-online": [Shield, Type, Sparkles],
  "write-on-photo-online": [PenLine, Type, Shield],
  "free-text-over-image-tool": [Layers, Type, Shield],
  "image-text-adder": [ImagePlus, Type, Shield],
  "add-watermark-to-image-online": [Shield, Type, Layers],
  "add-text-to-photos-for-instagram": [Sparkles, Type, Shield],
  "add-captions-to-images-online": [PenLine, Type, Shield],
  "add-logo-or-text-to-images": [Layers, Type, Shield],
  "client-side-text-overlay-tool": [Shield, Type, Layers],
  "add-text-to-image-with-fonts": [Type, Sparkles, Shield],
  "custom-text-placement-on-image": [Layers, Type, Shield],
  "professional-text-overlay-editor": [Shield, Type, Sparkles],
};

interface TextOverlayLandingViewProps {
  landingId: TextOverlayLandingId;
}

export function TextOverlayLandingView({
  landingId,
}: TextOverlayLandingViewProps) {
  const { language, dir } = useLanguage();
  const landing = getTextOverlayLandingEntry(language, landingId);
  const chrome = getTextOverlayLandingChrome(language);
  const benefitIcons =
    LANDING_BENEFIT_ICONS[landingId] ?? DEFAULT_BENEFIT_ICONS;
  const accent = TEXT_OVERLAY_LANDING_ACCENT;
  ...
  return (
    ...
    >
      <section className="mb-14 sm:mb-16">
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)] lg:items-end">
          <div className="max-w-2xl space-y-5">
            ...
            >
              <LandingCtaButton href={TEXT_OVERLAY_TOOL_HREF} accent={accent}>
                {landing.primaryCta}
              </LandingCtaButton>
      ...
      <TextOverlayLandingRelatedUseCases currentLandingId={landingId} />
