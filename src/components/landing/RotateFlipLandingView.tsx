...
import { RotateFlipLandingRelatedUseCases } from "@/components/landing/RotateFlipLandingRelatedUseCases";
import {
...
import {
  ROTATE_FLIP_LANDING_ACCENT,
  ROTATE_FLIP_TOOL_HREF,
  type RotateFlipLandingId,
} from "@/lib/rotateFlipLandings";
import {
  getRotateFlipLandingChrome,
  getRotateFlipLandingEntry,
...
const LANDING_BENEFIT_ICONS: Partial<
  Record<RotateFlipLandingId, readonly [LucideIcon, LucideIcon, LucideIcon]>
> = {
  "rotate-image-online": [Shield, RotateCw, Zap],
  "flip-image-online": [Shield, Zap, Sparkles],
  "mirror-image-online": [Shield, FlipHorizontal, Zap],
  "free-photo-rotator-and-flipper": [BadgeCheck, RotateCw, FlipHorizontal],
  "flip-photo-horizontally-and-vertically": [
    Shield,
    FlipHorizontal,
    FlipVertical,
  ],
  "rotate-image-90-degrees": [Shield, RotateCw, RotateCcw],
  "fix-upside-down-pictures-online": [Shield, RotateCw, Zap],
  "mirror-selfie-online": [Shield, FlipHorizontal, Sparkles],
  "batch-rotate-images-online": [BadgeCheck, RotateCw, Zap],
  "lossless-image-rotation-tool": [Shield, RotateCw, Sparkles],
  "batch-flip-photos-tool": [BadgeCheck, FlipHorizontal, Zap],
  "client-side-image-rotator": [Shield, RotateCw, Zap],
  "no-upload-photo-flip-tool": [Shield, FlipHorizontal, Zap],
  "private-browser-image-mirror": [Shield, FlipHorizontal, Sparkles],
  "secure-image-rotation-online": [Shield, RotateCw, BadgeCheck],
};

interface RotateFlipLandingViewProps {
  landingId: RotateFlipLandingId;
}

export function RotateFlipLandingView({
  landingId,
}: RotateFlipLandingViewProps) {
  const { language, dir } = useLanguage();
  const landing = getRotateFlipLandingEntry(language, landingId);
  const chrome = getRotateFlipLandingChrome(language);
  const benefitIcons =
    LANDING_BENEFIT_ICONS[landingId] ?? DEFAULT_BENEFIT_ICONS;
  const accent = ROTATE_FLIP_LANDING_ACCENT;
  ...
  return (
    ...
    >
      <section className="mb-14 sm:mb-16">
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)] lg:items-end">
          <div className="max-w-2xl space-y-5">
            ...
            >
              <LandingCtaButton href={ROTATE_FLIP_TOOL_HREF} accent={accent}>
                {landing.primaryCta}
              </LandingCtaButton>
      ...
      <RotateFlipLandingRelatedUseCases currentLandingId={landingId} />
