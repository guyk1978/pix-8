...
import { CropperLandingRelatedUseCases } from "@/components/landing/CropperLandingRelatedUseCases";
import {
...
import {
  CROPPER_LANDING_ACCENT,
  CROPPER_TOOL_HREF,
  type CropperLandingId,
} from "@/lib/cropperLandings";
import {
  getCropperLandingChrome,
  getCropperLandingEntry,
} from "@/lib/cropperLandingsLocale";
import { Aperture, BadgeCheck, CloudOff, Crop, Cpu, Crosshair, Focus, Lock, Monitor, Proportions, Ratio, RectangleHorizontal, Ruler, Scissors, Shield, ShoppingBag, Sparkles, Square, Zap, type LucideIcon } from "lucide-react";
import type { CSSProperties } from "react";

const DEFAULT_BENEFIT_ICONS = [Shield, Zap, Sparkles] as const;

const LANDING_BENEFIT_ICONS: Record<
  CropperLandingId,
  readonly [LucideIcon, LucideIcon, LucideIcon]
> = {
  "crop-image-online": [Shield, Zap, Crop],
  "free-image-cropper": [Shield, Zap, BadgeCheck],
  "crop-photos-to-size": [Shield, Zap, Ruler],
  "image-cutter-online": [Shield, Zap, Scissors],
  "crop-image-to-square": [Shield, Zap, Square],
  "crop-image-to-16-9": [Shield, Zap, RectangleHorizontal],
  "crop-image-to-4-3": [Shield, Zap, Ratio],
  "free-aspect-ratio-image-cropper": [Shield, Zap, Proportions],
  "crop-image-without-quality-loss": [Shield, Zap, Focus],
  "precision-image-cropper": [Shield, Zap, Crosshair],
  "crop-image-for-ecommerce-product-photos": [Shield, Zap, ShoppingBag],
  "professional-photo-cropper": [Shield, Zap, Aperture],
  "client-side-image-cropper": [Shield, Zap, Cpu],
  "privacy-focused-image-cutter": [Shield, Zap, Lock],
  "no-upload-image-cropper": [Shield, Zap, CloudOff],
  "browser-based-image-cropper-tool": [Shield, Zap, Monitor],
};

interface CropperLandingViewProps {
  landingId: CropperLandingId;
}

export function CropperLandingView({ landingId }: CropperLandingViewProps) {
  const { language, dir } = useLanguage();
  const landing = getCropperLandingEntry(language, landingId);
  const chrome = getCropperLandingChrome(language);
  const benefitIcons =
    LANDING_BENEFIT_ICONS[landingId] ?? DEFAULT_BENEFIT_ICONS;
  const accent = CROPPER_LANDING_ACCENT;
  ...
  return (
    ...
    >
      <section className="mb-14 sm:mb-16">
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)] lg:items-end">
          <div className="max-w-2xl space-y-5">
            ...
            >
              <LandingCtaButton href={CROPPER_TOOL_HREF} accent={accent}>
                {landing.primaryCta}
              </LandingCtaButton>
      ...
      <section className="mb-14 sm:mb-16">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {landing.benefits.map(({ title, body }, index) => {
            const Icon: LucideIcon = benefitIcons[index] ?? Shield;
            return (
              <article
                key={title}
      ...
      <CropperLandingRelatedUseCases currentLandingId={landingId} />
