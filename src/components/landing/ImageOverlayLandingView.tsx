...
import { ImageOverlayLandingRelatedUseCases } from "@/components/landing/ImageOverlayLandingRelatedUseCases";
import {
...
import {
  IMAGE_OVERLAY_LANDING_ACCENT,
  IMAGE_OVERLAY_TOOL_HREF,
  type ImageOverlayLandingId,
} from "@/lib/imageOverlayLandings";
import {
  getImageOverlayLandingChrome,
  getImageOverlayLandingEntry,
} from "@/lib/imageOverlayLandingsLocale";
import { Layers, Shield, Sparkles, type LucideIcon } from "lucide-react";
import type { CSSProperties } from "react";

const DEFAULT_BENEFIT_ICONS = [Shield, Layers, Sparkles] as const;

interface ImageOverlayLandingViewProps {
  landingId: ImageOverlayLandingId;
}

export function ImageOverlayLandingView({
  landingId,
}: ImageOverlayLandingViewProps) {
  const { language, dir } = useLanguage();
  const landing = getImageOverlayLandingEntry(language, landingId);
  const chrome = getImageOverlayLandingChrome(language);
  const accent = IMAGE_OVERLAY_LANDING_ACCENT;
  ...
  return (
    ...
    >
      <section className="mb-14 sm:mb-16">
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)] lg:items-end">
          <div className="max-w-2xl space-y-5">
            ...
            >
              <LandingCtaButton href={IMAGE_OVERLAY_TOOL_HREF} accent={accent}>
                {landing.primaryCta}
              </LandingCtaButton>
      ...
      <ImageOverlayLandingRelatedUseCases currentLandingId={landingId} />
