...
import { ColorPickerLandingRelatedUseCases } from "@/components/landing/ColorPickerLandingRelatedUseCases";
import {
...
import {
  COLOR_PICKER_LANDING_ACCENT,
  COLOR_PICKER_TOOL_HREF,
  type ColorPickerLandingId,
} from "@/lib/colorpickerLandings";
import {
  getColorPickerLandingChrome,
  getColorPickerLandingEntry,
} from "@/lib/colorpickerLandingsLocale";
import { Crosshair, Eye, Pipette, type LucideIcon } from "lucide-react";
import type { CSSProperties } from "react";

const DEFAULT_BENEFIT_ICONS = [Crosshair, Eye, Pipette] as const;

interface ColorPickerLandingViewProps {
  landingId: ColorPickerLandingId;
}

export function ColorPickerLandingView({
  landingId,
}: ColorPickerLandingViewProps) {
  const { language, dir } = useLanguage();
  const landing = getColorPickerLandingEntry(language, landingId);
  const chrome = getColorPickerLandingChrome(language);
  const accent = COLOR_PICKER_LANDING_ACCENT;
  ...
  return (
    ...
    >
      <section className="mb-14 sm:mb-16">
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)] lg:items-end">
          <div className="max-w-2xl space-y-5">
            ...
            >
              <LandingCtaButton href={COLOR_PICKER_TOOL_HREF} accent={accent}>
                {landing.primaryCta}
              </LandingCtaButton>
      ...
      <section className="mb-14 sm:mb-16">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {landing.benefits.map(({ title, body }, index) => {
            const Icon: LucideIcon = DEFAULT_BENEFIT_ICONS[index] ?? Pipette;
            return (
              <article
                key={title}
      ...
      <ColorPickerLandingRelatedUseCases currentLandingId={landingId} />
