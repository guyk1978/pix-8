...
import { PaletteExtractorLandingRelatedUseCases } from "@/components/landing/PaletteExtractorLandingRelatedUseCases";
import {
...
import {
  ...
  getPaletteExtractorLandingEntry,
} from "@/lib/paletteextractorLandingsLocale";
import { Copy, HardDrive, Palette, type LucideIcon } from "lucide-react";
import type { CSSProperties } from "react";

const DEFAULT_BENEFIT_ICONS = [HardDrive, Palette, Copy] as const;

interface PaletteExtractorLandingViewProps {
  landingId: PaletteExtractorLandingId;
}

export function PaletteExtractorLandingView({
  landingId,
}: PaletteExtractorLandingViewProps) {
  ...
  const accent = PALETTE_EXTRACTOR_LANDING_ACCENT;
  ...
  return (
    ...
    >
      <section className="mb-14 sm:mb-16">
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)] lg:items-end">
          <div className="max-w-2xl space-y-5">
            ...
            >
              <LandingCtaButton href={PALETTE_EXTRACTOR_TOOL_HREF} accent={accent}>
                {landing.primaryCta}
              </LandingCtaButton>
      ...
      <section className="mb-14 sm:mb-16">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {landing.benefits.map(({ title, body }, index) => {
            const Icon: LucideIcon = DEFAULT_BENEFIT_ICONS[index] ?? Palette;
            return (
              <article
                key={title}
      ...
      <PaletteExtractorLandingRelatedUseCases currentLandingId={landingId} />
