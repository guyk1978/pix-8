...
import {
  ...
  getRelatedImageCollageLandingPages,
...
interface ImageCollageLandingRelatedUseCasesProps {
  currentLandingId: ImageCollageLandingId;
}

export function ImageCollageLandingRelatedUseCases({
  currentLandingId,
}: ImageCollageLandingRelatedUseCasesProps) {
  const { language } = useLanguage();
  const relatedPages = getRelatedImageCollageLandingPages(
    currentLandingId,
    language,
  );
                ...
                <li key={page.path}>
                  <AppLink href={page.path} className={linkCardClassName}>
                    <h3
                      ...
                      style={{ color: IMAGE_COLLAGE_LANDING_ACCENT }}
                    >
                      {page.linkTitle}
                    </h3>
