...
import { ImageFiltersLandingRelatedUseCases } from "@/components/landing/ImageFiltersLandingRelatedUseCases";
import {
...
import {
  IMAGE_FILTERS_LANDING_ACCENT,
  IMAGE_FILTERS_TOOL_HREF,
  type ImageFiltersLandingId,
} from "@/lib/imagefiltersLandings";
import {
  getImageFiltersLandingChrome,
  getImageFiltersLandingEntry,
} from "@/lib/imagefiltersLandingsLocale";
import { Shield, SlidersHorizontal, Sparkles, type LucideIcon } from "lucide-react";
import type { CSSProperties } from "react";

const DEFAULT_BENEFIT_ICONS = [Shield, SlidersHorizontal, Sparkles] as const;

interface ImageFiltersLandingViewProps {
  landingId: ImageFiltersLandingId;
}

export function ImageFiltersLandingView({
  landingId,
}: ImageFiltersLandingViewProps) {
  const { language, dir } = useLanguage();
  const landing = getImageFiltersLandingEntry(language, landingId);
  const chrome = getImageFiltersLandingChrome(language);
  const accent = IMAGE_FILTERS_LANDING_ACCENT;
  ...
  return (
    ...
    >
      <section className="mb-14 sm:mb-16">
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)] lg:items-end">
          <div className="max-w-2xl space-y-5">
            ...
            >
              <LandingCtaButton href={IMAGE_FILTERS_TOOL_HREF} accent={accent}>
                {landing.primaryCta}
              </LandingCtaButton>
      ...
      <section className="mb-14 sm:mb-16">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {landing.benefits.map(({ title, body }, index) => {
            const Icon: LucideIcon = DEFAULT_BENEFIT_ICONS[index] ?? Shield;
            return (
              <article
                key={title}
      ...
      <ImageFiltersLandingRelatedUseCases currentLandingId={landingId} />
