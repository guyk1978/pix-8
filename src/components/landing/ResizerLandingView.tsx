...
import { ResizerLandingRelatedUseCases } from "@/components/landing/ResizerLandingRelatedUseCases";
import {
...
import {
  RESIZER_LANDING_ACCENT,
  RESIZER_TOOL_HREF,
  type ResizerLandingId,
} from "@/lib/resizerLandings";
import {
  getResizerLandingChrome,
  getResizerLandingEntry,
} from "@/lib/resizerLandingsLocale";
import { Shield, Sparkles, Zap, Maximize2, Ruler, BadgeCheck, Layers, Smartphone, UserCircle, PanelTop, Share2, Grid3x3, Ratio, Focus, Code2, Cpu, Lock, CloudOff, Monitor, type LucideIcon } from "lucide-react";
...
> = {
  "resize-image-online": [Shield, Zap, Maximize2],
  "change-image-dimensions": [Shield, Zap, Ruler],
  "image-resizer-free": [Shield, Zap, BadgeCheck],
  "batch-image-resizer": [Shield, Zap, Layers],
  "resize-image-for-instagram": [Shield, Zap, Smartphone],
  "image-resizer-for-linkedin-profile": [Shield, Zap, UserCircle],
  "resize-photos-for-facebook-covers": [Shield, Zap, PanelTop],
  "image-dimensions-for-social-media": [Shield, Zap, Share2],
  "resize-image-to-pixels": [Shield, Zap, Grid3x3],
  "maintain-aspect-ratio-image-resizer": [Shield, Zap, Ratio],
  "resize-image-without-quality-loss": [Shield, Zap, Focus],
  "image-resizer-for-web-developers": [Shield, Zap, Code2],
  "client-side-image-resizer": [Shield, Zap, Cpu],
  "privacy-focused-photo-resizer": [Shield, Zap, Lock],
  "no-upload-image-resizer": [Shield, Zap, CloudOff],
  "browser-based-photo-resizer": [Shield, Zap, Monitor],
};

interface ResizerLandingViewProps {
  landingId: ResizerLandingId;
}

export function ResizerLandingView({ landingId }: ResizerLandingViewProps) {
  const { language, dir } = useLanguage();
  const landing = getResizerLandingEntry(language, landingId);
  const chrome = getResizerLandingChrome(language);
  const benefitIcons =
    LANDING_BENEFIT_ICONS[landingId] ?? DEFAULT_BENEFIT_ICONS;
  const accent = RESIZER_LANDING_ACCENT;
  ...
  return (
    ...
    >
      <section className="mb-14 sm:mb-16">
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)] lg:items-end">
          <div className="max-w-2xl space-y-5">
            ...
            >
              <LandingCtaButton href={RESIZER_TOOL_HREF} accent={accent}>
                {landing.primaryCta}
              </LandingCtaButton>
      ...
      <ResizerLandingRelatedUseCases currentLandingId={landingId} />
