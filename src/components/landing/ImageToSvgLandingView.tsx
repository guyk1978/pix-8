...
import { ImageToSvgLandingRelatedUseCases } from "@/components/landing/ImageToSvgLandingRelatedUseCases";
import {
...
import {
  ...
  getImageToSvgLandingEntry,
} from "@/lib/imagetosvgLandingsLocale";
import { Download, PenLine, Shapes, type LucideIcon } from "lucide-react";
import type { CSSProperties } from "react";

const DEFAULT_BENEFIT_ICONS = [Shapes, PenLine, Download] as const;

interface ImageToSvgLandingViewProps {
  landingId: ImageToSvgLandingId;
}

export function ImageToSvgLandingView({
  landingId,
}: ImageToSvgLandingViewProps) {
  const { language, dir } = useLanguage();
  const landing = getImageToSvgLandingEntry(language, landingId);
  const chrome = getImageToSvgLandingChrome(language);
  const accent = IMAGE_TO_SVG_LANDING_ACCENT;
  ...
  return (
    ...
    >
      <section className="mb-14 sm:mb-16">
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)] lg:items-end">
          <div className="max-w-2xl space-y-5">
            ...
            >
              <LandingCtaButton href={IMAGE_TO_SVG_TOOL_HREF} accent={accent}>
                {landing.primaryCta}
              </LandingCtaButton>
      ...
      <section className="mb-14 sm:mb-16">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {landing.benefits.map(({ title, body }, index) => {
            const Icon: LucideIcon = DEFAULT_BENEFIT_ICONS[index] ?? Shapes;
            return (
              <article
                key={title}
      ...
      <ImageToSvgLandingRelatedUseCases currentLandingId={landingId} />
