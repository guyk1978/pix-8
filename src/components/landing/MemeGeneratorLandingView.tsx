...
import { MemeGeneratorLandingRelatedUseCases } from "@/components/landing/MemeGeneratorLandingRelatedUseCases";
import {
...
import {
  MEME_GENERATOR_LANDING_ACCENT,
  MEME_GENERATOR_TOOL_HREF,
  type MemeGeneratorLandingId,
} from "@/lib/memeGeneratorLandings";
import {
  getMemeGeneratorLandingChrome,
  getMemeGeneratorLandingEntry,
} from "@/lib/memeGeneratorLandingsLocale";
import { Image, Laugh, Shield, type LucideIcon } from "lucide-react";
import type { CSSProperties } from "react";

const DEFAULT_BENEFIT_ICONS = [Shield, Laugh, Image] as const;

interface MemeGeneratorLandingViewProps {
  landingId: MemeGeneratorLandingId;
}

export function MemeGeneratorLandingView({
  landingId,
}: MemeGeneratorLandingViewProps) {
  const { language, dir } = useLanguage();
  const landing = getMemeGeneratorLandingEntry(language, landingId);
  const chrome = getMemeGeneratorLandingChrome(language);
  const accent = MEME_GENERATOR_LANDING_ACCENT;
  ...
  return (
    ...
    >
      <section className="mb-14 sm:mb-16">
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)] lg:items-end">
          <div className="max-w-2xl space-y-5">
            ...
            >
              <LandingCtaButton href={MEME_GENERATOR_TOOL_HREF} accent={accent}>
                {landing.primaryCta}
              </LandingCtaButton>
      ...
      <MemeGeneratorLandingRelatedUseCases currentLandingId={landingId} />
