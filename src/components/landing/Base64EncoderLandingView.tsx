...
import { Base64EncoderLandingRelatedUseCases } from "@/components/landing/Base64EncoderLandingRelatedUseCases";
import {
...
import {
  ...
  getBase64EncoderLandingEntry,
} from "@/lib/base64encoderLandingsLocale";
import { Binary, ClipboardCopy, HardDrive, type LucideIcon } from "lucide-react";
import type { CSSProperties } from "react";

const DEFAULT_BENEFIT_ICONS = [HardDrive, ClipboardCopy, Binary] as const;

interface Base64EncoderLandingViewProps {
  landingId: Base64EncoderLandingId;
}

export function Base64EncoderLandingView({
  landingId,
}: Base64EncoderLandingViewProps) {
  const { language, dir } = useLanguage();
  const landing = getBase64EncoderLandingEntry(language, landingId);
  const chrome = getBase64EncoderLandingChrome(language);
  const accent = BASE64_ENCODER_LANDING_ACCENT;
  ...
  return (
    ...
    >
      <section className="mb-14 sm:mb-16">
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)] lg:items-end">
          <div className="max-w-2xl space-y-5">
            ...
            >
              <LandingCtaButton href={BASE64_ENCODER_TOOL_HREF} accent={accent}>
                {landing.primaryCta}
              </LandingCtaButton>
      ...
      <section className="mb-14 sm:mb-16">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {landing.benefits.map(({ title, body }, index) => {
            const Icon: LucideIcon = DEFAULT_BENEFIT_ICONS[index] ?? Binary;
            return (
              <article
                key={title}
      ...
      <Base64EncoderLandingRelatedUseCases currentLandingId={landingId} />
