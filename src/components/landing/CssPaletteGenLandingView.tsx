...
import { CssPaletteGenLandingRelatedUseCases } from "@/components/landing/CssPaletteGenLandingRelatedUseCases";
import {
...
import {
  ...
  getCssPaletteGenLandingEntry,
} from "@/lib/csspalettegenLandingsLocale";
import { Braces, Code, ShieldCheck, type LucideIcon } from "lucide-react";
import type { CSSProperties } from "react";

const DEFAULT_BENEFIT_ICONS = [Braces, Code, ShieldCheck] as const;

interface CssPaletteGenLandingViewProps {
  landingId: CssPaletteGenLandingId;
}

export function CssPaletteGenLandingView({
  landingId,
}: CssPaletteGenLandingViewProps) {
  const { language, dir } = useLanguage();
  const landing = getCssPaletteGenLandingEntry(language, landingId);
  const chrome = getCssPaletteGenLandingChrome(language);
  const accent = CSS_PALETTE_GEN_LANDING_ACCENT;
  ...
  return (
    ...
    >
      <section className="mb-14 sm:mb-16">
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)] lg:items-end">
          <div className="max-w-2xl space-y-5">
            ...
            >
              <LandingCtaButton href={CSS_PALETTE_GEN_TOOL_HREF} accent={accent}>
                {landing.primaryCta}
              </LandingCtaButton>
      ...
      <section className="mb-14 sm:mb-16">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {landing.benefits.map(({ title, body }, index) => {
            const Icon: LucideIcon = DEFAULT_BENEFIT_ICONS[index] ?? Braces;
            return (
              <article
                key={title}
      ...
      <CssPaletteGenLandingRelatedUseCases currentLandingId={landingId} />
