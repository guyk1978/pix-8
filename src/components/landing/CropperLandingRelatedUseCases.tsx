...
interface CropperLandingRelatedUseCasesProps {
  currentLandingId: CropperLandingId;
}

export function CropperLandingRelatedUseCases({
  currentLandingId,
}: CropperLandingRelatedUseCasesProps) {
  const { language } = useLanguage();
  const relatedPages = getRelatedCropperLandingPages(
    currentLandingId,
    language,
  );
                ...
                <li key={page.path}>
                  <AppLink href={page.path} className={linkCardClassName}>
                    <h3
                      ...
                      style={{ color: CROPPER_LANDING_ACCENT }}
                    >
                      {page.linkTitle}
                    </h3>
