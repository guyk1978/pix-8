...
import { WatermarkLandingRelatedUseCases } from "@/components/landing/WatermarkLandingRelatedUseCases";
import {
...
import {
  ...
  getWatermarkLandingEntry,
} from "@/lib/watermarkLandingsLocale";
import { Shield, Stamp, Type, type LucideIcon } from "lucide-react";
import type { CSSProperties } from "react";

const DEFAULT_BENEFIT_ICONS = [Shield, Stamp, Type] as const;

interface WatermarkLandingViewProps {
  landingId: WatermarkLandingId;
}

export function WatermarkLandingView({
  landingId,
}: WatermarkLandingViewProps) {
  const { language, dir } = useLanguage();
  const landing = getWatermarkLandingEntry(language, landingId);
  const chrome = getWatermarkLandingChrome(language);
  const accent = WATERMARK_LANDING_ACCENT;
  ...
  return (
    ...
    >
      <section className="mb-14 sm:mb-16">
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)] lg:items-end">
          <div className="max-w-2xl space-y-5">
            ...
            >
              <LandingCtaButton href={WATERMARK_TOOL_HREF} accent={accent}>
                {landing.primaryCta}
              </LandingCtaButton>
      ...
      <WatermarkLandingRelatedUseCases currentLandingId={landingId} />
