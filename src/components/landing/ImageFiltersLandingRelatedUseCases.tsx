...
import {
  ...
  getRelatedImageFiltersLandingPages,
...
interface ImageFiltersLandingRelatedUseCasesProps {
  currentLandingId: ImageFiltersLandingId;
}

export function ImageFiltersLandingRelatedUseCases({
  currentLandingId,
}: ImageFiltersLandingRelatedUseCasesProps) {
  const { language } = useLanguage();
  const relatedPages = getRelatedImageFiltersLandingPages(
    currentLandingId,
    language,
  );
                ...
                <li key={page.path}>
                  <AppLink href={page.path} className={linkCardClassName}>
                    <h3
                      ...
                      style={{ color: IMAGE_FILTERS_LANDING_ACCENT }}
                    >
                      {page.linkTitle}
                    </h3>
